...
const DeviceCard = (props: Props) => {
    ...
    const cardClass = twMerge(classNames(' w-[400px] min-h-[200px] h-auto m-2 border-0 text-white',{
      'bg-secondary': !props.device.isOffline,
      "bg-red-500": props.device.isOffline
    }))
  ...
  return (
    <Card className={cardClass}>
        <CardHeader className='relative place-items-center'>
            ...
            <CardDescription>{DEVICE_STRINGS.STATUS}{props.device.isOffline ? "Offline" : "Online"}</CardDescription>
        ...
        <CardContent className='flex flex-col text-lg font-bold'>
            <span>{DEVICE_STRINGS.TEMPERATURE} {props.device.temp} {UNITS.CELSIUS}</span>
            <span>{DEVICE_STRINGS.HUMIDITY} {props.device.humidity}{UNITS.PERCENTAGE}</span>
        </CardContent>
    </Card>
  )
}
